  ...
  for(t in 2:Rounds) {
    ...
    for(n in 1:NumPlayer){
      ...
      for(j in 1:as.numeric(M[n])){
        ...
        q[n,j,t] = (1-phi)*q[n,j,(t-1)]
        
        if (as.numeric(k[n,t]) == j) {
          #update the propensities based on the reward observed
          q[n,j,t] =  q[n,j,t]+as.numeric(R[n])*(1-eps)        
        } else{
          
          if (as.numeric(M[n]) <= 2) {
            
            if ((j == (as.numeric(k[n,t])+1))|(j == (as.numeric(k[n,t])-1))) {
              #update the propensities based on the reward observed
              q[n,j,t] =  q[n,j,t]+as.numeric(R[n])*(eps/2)        
            }          
            
          } else {
            q[n,j,t] =  q[n,j,t]+as.numeric(R[n])*(eps/(as.numeric(M[n])-1))
          }  
