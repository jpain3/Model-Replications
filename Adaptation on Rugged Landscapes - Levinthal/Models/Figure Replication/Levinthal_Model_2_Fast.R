...
for (q in 1:length(K_values)) {
  ...
  for (r in 1:replications) {
    ...
    #Part 2 Specific: Make everyone start out the same:
    EntityMatrix[,]=EntityMatrix[1,]
...
plot((0:Horizon),Unique_Firms_avg[,2], typ = "l", pch=4, col = "black", lwd=2, lty =1,
...
plot((0:Horizon),Unique_Firms_avg[,1], typ = "l", pch=19, col = "black", lwd=1,
     xlim=c(0,Horizon), ylim=c(0,max(Unique_Firms_avg)*1.1),
     xlab = "Time", ylab = "",
