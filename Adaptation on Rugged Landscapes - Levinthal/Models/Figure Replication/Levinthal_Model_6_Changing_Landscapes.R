...
for (q in 1:length(K_values)) {
  ...
  for (r in 1:replications) {
    ...
    for (t in 2:(Horizon+1)) {
      ...
      #Reshuffle at ChangeTime
      if (t == ChangeTime) {
        
        #Check if Reshuffle is enabled
        if (Reshuffle == TRUE) {
          
          print("SHUFFLE! Re-determining Permuation Values...")
          
          #Overwrite the Fitness Table for the number of shifted parameters, limited by the maximum allowable
          FitTable[1:min(ChangeParam,NumFitness),"Fitness"] = runif(min(ChangeParam,NumFitness))
          
          #Determine the fitness for every possible entity
          
          print("SHUFFLE! Re-populating Fitness Lookup Table...")
          FullTable$Fitness = rep(0,nrow(FullTable))
          ...
          #Look up nearest neighbors and the one with the maximum fitness value
          print(paste("SHUFFLE! Re-generating list of maximum fitness neighbors..."))
          Max_Neighbor = function(EntityString) {
            DiffMatrix = t(abs(apply(FullTable[,1:N],1,'-',unname(unlist(EntityString)))))
            DiffVector = apply(DiffMatrix,1,sum)
            SubsetDiffFitness = which(DiffVector == 1)
...
plot((ChangeTime:Horizon),SurvivorCount_avg[((ChangeTime):(Horizon)),1], typ = "l", pch=19, col = "black", lwd=1,
     xlim=c(ChangeTime,Horizon), ylim=c(0,NumEntity),
     xlab = "Time", ylab = "% Surviving Incumbents",
     main = "Survival of Firms (Change in Fitness of Contribution of Five Permutations)"
)
