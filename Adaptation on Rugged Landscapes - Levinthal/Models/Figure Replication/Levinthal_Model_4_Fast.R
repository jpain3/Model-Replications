...
for (q in 1:length(K_values)) {
  ...
  DistributionArray = array(dim = c(2^N,Horizon+1))
  ...
  for (r in 1:replications) {
    ...
    DistributionArray[IndexOfEntities,1] = CountOfEntities
    ...
    for (t in 2:(Horizon+1)) {
      ...
      for (n in 1:nrow(EntityMatrix)) {
        ...
        #Get the entity that is looking for higher fitness values
        ...
        MaxNeighbor = FullTable[FullTableNeighbor,]
        
        #String updating
        
        #Determine which has the highest fitness - original string, long distance (Random) string, or neighbor string
        MaxFitnessIndex = which.max(c(EntityMatrix[n,"Fitness"],RandEntity$Fitness,MaxNeighbor$Fitness))
        
        #Update the Entity with the one with the highest fitness (note, on full ties, the original string wins)
        MaxFitnessEntity = rbind(EntityMatrix[n,],RandEntity,MaxNeighbor)[MaxFitnessIndex,]
        EntityMatrix[n,] = MaxFitnessEntity
    ...
    IndexOfEntities = as.integer(rownames(table(match(EntityMatrix$MatchString,FullTable$MatchString))))
    CountOfEntities = unname(table(match(EntityMatrix$MatchString,FullTable$MatchString)))
    DistributionArray[IndexOfEntities,t] = CountOfEntities
