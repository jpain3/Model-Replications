...
#Model replictions to average over
replications = 20
...
for (q in 1:length(K_values)) {
  ...
  for (r in 1:replications) {
    ...
    FullTableMean = mean(FullTable$Fitness)
    FullTableSD = sd(FullTable$Fitness)
    ...
    for (t in 2:(Horizon+1)) {
      ...
      if (OrgAdapation == TRUE) {
        ...
        for (n in 1:nrow(EntityMatrix)) {
          ...
          if (Noisy_Search == TRUE) {
          
            #Get degree of difference between this random string and the Entity
            DiffVector = t(abs(apply(RandEntity[,1:N],1,'-',EntityString)))
            DegDiff = apply(DiffVector,1,sum)
            
            RandSD = (max(DegDiff,0)/N)*FullTableSD
            
            Noisy_RandEntity = RandEntity
            Noisy_RandEntity$Fitness = rnorm(1,RandEntity$Fitness,RandSD)
           
          } else {
            Noisy_RandEntity = RandEntity
            
          } #End Noisy Search Block
          
          
          if (SharedResources == TRUE) {
            
            FloorFitness = RandEntity$Fitness - SDMultiplier*FullTableSD
            
            #Get current number of incumbents in the found position
            EntityTable = table(EntityMatrix$MatchString)
            NumIncumbents = unname(EntityTable[names(EntityTable) == RandEntity$MatchString])
            
            if (length(NumIncumbents) > 0) {
              NumIncumbents = NumIncumbents
              
            } else {
              NumIncumbents = 0
            }
            
            #Determine the expected fitness value that would be realized if this Entity moved to that position
            RealizedFitness = (RandEntity$Fitness - FloorFitness)*exp(-(NumIncumbents-1+1)) + FloorFitness
            Noisy_RandEntity$Fitness = RealizedFitness
            
          } #End First Shared Resources Block
          
          
          #If superior, then update the Entity
          if (Noisy_RandEntity$Fitness > EntityMatrix[n,"Fitness"] ) {
            
            #Realize whatever the actual fitness level was
            ...
            if (Noisy_Search == TRUE) {
              RandSD = (1/N)*FullTableSD
              Rand_NeighborFitness = rnorm(1,FullTable[FullTableNeighbor,"Fitness"],RandSD)
              
            } else {
              Rand_NeighborFitness = FullTable[FullTableNeighbor,"Fitness"]
            }
            
            
            if (Rand_NeighborFitness > FullTable[FullTableIndex,"Fitness"]) {
              #Make the swap
              EntityMatrix[n,] = FullTable[FullTableNeighbor,]
            }
            
          } #End update Entity Block
          
          #If sharing resources, update the entity table fitness to reflect current number of incumbents at each position
          if (SharedResources == TRUE) {
            
            #Determine the number of Entities that share resources
            EntityTable = table(EntityMatrix$MatchString)
            EntityMatchStrings = unlist(names(EntityTable))
            EnityMatchStringCounts = as.vector(unname(unlist(EntityTable)))
            lookupTbl = data.frame(EntityMatchStrings,EnityMatchStringCounts)
            NumIncumbents = lookupTbl$EnityMatchStringCounts[match(EntityMatrix$MatchString,lookupTbl$EntityMatchStrings)]
            
            #Adjust the fitness realized by the entities based on how many are sharing resources
            NoIncumbentFitness = FullTable[match(EntityMatrix$MatchString,FullTable$MatchString),"Fitness"]
            FloorFitnessVector = NoIncumbentFitness - SDMultiplier*FullTableSD
            AdjustedFitness = (NoIncumbentFitness-FloorFitnessVector)*exp(-(NumIncumbents-1))+FloorFitnessVector
            EntityMatrix$Fitness = AdjustedFitness  
          
          } #End Second Shared Resources Block
...
plot((0:Horizon),Unique_Firms_avg[,1], typ = "l", pch=19, col = "black", lwd=1,
     ...
     main = "Selection, Joint Adaptation, Resource Sharing (SDMult=3)"
)
